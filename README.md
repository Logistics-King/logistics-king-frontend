...
실제 화면 캡처는 `docs/screenshots` 폴더에 추가하면 아래 이미지 자리와 연결해서 사용할 수 있습니다.
...
현재 `agency01` 샘플 계정 기준으로는 조회 가능한 공개 일감이 없어서 빈 목록 화면이 보입니다.
공개 일감이 생기면 이 화면에 계약 요청 카드가 표시됩니다.
...
현재 캡처는 공개 일감이 없는 상태라 제안 등록 버튼이 없는 빈 목록 화면입니다.
...
현재 화면은 기본 메뉴와 계약 확인 화면 중심으로 구성되어 있습니다.

스크린샷 파일: `docs/screenshots/driver-home.png`
...
스크린샷 파일: `docs/screenshots/admin-home.png`
